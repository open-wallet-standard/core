...
describe('@open-wallet-standard/viem', () => {
  ...
  before(() => {
    vaultDir = mkdtempSync(join(tmpdir(), 'ows-viem-test-'));
    createWallet(walletName, undefined, 12, vaultDir);
  });
  ...
  it('signMessage with string returns hex signature', async () => {
    const account = owsToViemAccount(walletName, { vaultPath: vaultDir });
    const sig = await account.signMessage({ message: 'hello' });
    assert.match(sig, /^0x[0-9a-fA-F]+$/);
  });

  it('signMessage produces deterministic signatures', async () => {
    const account = owsToViemAccount(walletName, { vaultPath: vaultDir });
  ...
  it('signMessage matches OWS SDK direct call', async () => {
    ...
    const directResult = owsSignMessage(
      walletName, 'eip155:1', 'parity-check',
      undefined, undefined, undefined, vaultDir
    );
    const directSig = directResult.signature.startsWith('0x')
      ? directResult.signature
      : `0x${directResult.signature}`;
  ...
  it('throws when no EVM account in wallet', () => {
    // This would require a wallet with no eip155 accounts — hard to create
    // since OWS always derives all chains. Skip for now.
  });
});
